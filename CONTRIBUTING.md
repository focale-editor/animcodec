# Contributing to Animcodec

Thanks for helping improve Animcodec. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/animcodec/issues) before opening a new one.
...
Animcodec follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Flutter SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/animcodec.git
cd animcodec
flutter pub get
```
...
Animation codec changes should include focused round-trip, malformed-input, limit, frame-composition, timing, loop, palette, and disposal-method tests where applicable.
